...
abstract class GenerateResourcesTask : DefaultTask() {
    ...
    fun generate() {
        ...
        val outputDirs = if (settings.mergeResources.get() == false) {
            settings.mappedResourcesPath.get().mapValues {
                project.layout.buildDirectory.dir("generated/kotlin/${it.value.targetName}").get().asFile
            }
        } else mapOf("commonMain/resources" to project.layout.buildDirectory.dir("generated/kotlin").get().asFile)
        outputDirs.forEach {
            val outputFile = File(it.value.apply { mkdirs() }, "_RKMbed_GeneratedResources_x_x_x_1_2.kt")
            val generatedCode = buildString {
                ...
                outputDirs.forEach { (sourceSet, _) ->
